...
created: 2026-09-13
consumers: [currency-board, weekly-reflection, review-pass]
---
...
A Board card in OpenAugi or Research Engineer is a PMOC and that PMOC is `#status/active`; if they disagree the Board wins. Backlog sub-lists are allowed where an area has real sub-kinds, a few, never on the Board. A new column only when they name it.
...
- **The plugin cannot move a card between files.** So a cross-board move is either the user cutting and pasting the line, or an `aaa:` marker: `aaa: → board` appended to a Backlog card, or `aaa: → backlog` to a Board card, means *move this on the next pass*. Any session pointed at the boards, and the Sunday pass, performs the move — the card line moves intact to the bottom of the same-named column on the other board and the marker is removed. A card with any other `aaa:` text is feedback about that card, read and honored, never moved.
...
Part of the weekly reflection, after the priority ruling:

1. **Triage.** Each card in the Board's Triage column gets one proposed destination.
2. **Add.** The week's surfaced `[todo]` and `[idea]` items go to the Backlog with their source, as checkbox proposals.
3. **Over cap.** Any AMOC column past 3: one line naming the cards, then talk. Nothing nominated.
4. **Pull.** If they want to fill a slot, offer the Backlog cards their writing touched this week.
5. **Sync.** A Board card in OpenAugi or Research Engineer ↔ a `#status/active` PMOC. Fix the tag, not the board.
