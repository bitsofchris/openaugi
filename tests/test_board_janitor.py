...
# ── the weekly reflection: the second surface with `do` / `no` offers ─────

REFLECTION = """---
type: document
lens: weekly-reflection
---
- [ ] seen
...
Write your thoughts in [[WK - 26-09-20]] first, then tick.

- **Propose next week** `build · agent`
    ↳ Apply the `weekly-reflection` lens § Propose to this note.
    - [ ] do
    - [ ] no
    <!-- propose:weekly-propose-2026-09-20 -->
"""


@pytest.fixture
def reflection(vault):
    folder = vault / "OpenAugi" / "Research"
    folder.mkdir(parents=True)
    path = folder / "Weekly Reflection - 2026-09-20.md"
    path.write_text(REFLECTION, encoding="utf-8")
    return path


def test_reflection_note_is_recognised_by_path_shape_only(vault):
    from openaugi.pipeline.board_janitor import is_reflection_note

    research = vault / "OpenAugi" / "Research"
    assert is_reflection_note(research / "Weekly Reflection - 2026-09-20.md")
    assert not is_reflection_note(research / "Weekly Reflection - 2026-09-20.txt")
    assert not is_reflection_note(research / "Synthesis - Context Engineering.md")
    drafts = vault / "OpenAugi" / "Drafts"
    assert not is_reflection_note(drafts / "Weekly Reflection - 2026-09-20.md")


def test_ticking_do_on_the_reflection_dispatches_its_own_task(reflection, vault):
    from openaugi.pipeline.board_janitor import process_changed

    _tick_proposal(reflection, "do", "weekly-propose-2026-09-20")
    assert process_changed({str(reflection)}, vault) == 1

    task = vault / "OpenAugi" / "Tasks" / "reflection-2026-09-20-weekly-propose-2026-09-20.md"
    body = task.read_text(encoding="utf-8")
    assert "status: pending" in body
    assert "source: weekly-reflection" in body
    assert "proposal: weekly-propose-2026-09-20" in body
    assert "Proposed by the weekly reflection on 2026-09-20" in body
    assert "[[Weekly Reflection - 2026-09-20]]" in body
    # The brief he read is the brief the agent gets — and it only points at the lens.
    assert "Apply the `weekly-reflection` lens § Propose" in body

    text = reflection.read_text(encoding="utf-8")
    assert "- ✓ do → `reflection-2026-09-20-weekly-propose-2026-09-20.md`" in text
    assert "] no\n" not in text


def test_reflection_ticks_never_touch_board_state(reflection, vault):
    from openaugi.pipeline.board_janitor import sync_reflection

    _tick_proposal(reflection, "do", "weekly-propose-2026-09-20")
    sync_reflection(reflection, vault)
    assert not (vault / "OpenAugi" / "Board" / ".board-state.json").exists()
    record = json.loads(
        (vault / "OpenAugi" / "Capture" / "feedback-log.ndjson")
        .read_text(encoding="utf-8")
        .strip()
    ...
    assert record["source"] == "weekly-reflection-proposal"
    assert record["signal"] == "dispatched"
    # A board rebuild reads only board-sourced rows, so the Sunday tick is invisible to it.
    assert "weekly-propose-2026-09-20" not in rebuild_state(vault)["proposals"]


def test_reflection_sync_is_idempotent(reflection, vault):
    from openaugi.pipeline.board_janitor import sync_reflection

    _tick_proposal(reflection, "do", "weekly-propose-2026-09-20")
    assert sync_reflection(reflection, vault) == 1
    after_first = reflection.read_text(encoding="utf-8")
    assert sync_reflection(reflection, vault) == 0
    assert reflection.read_text(encoding="utf-8") == after_first


def test_declining_a_reflection_offer_writes_no_task(reflection, vault):
    from openaugi.pipeline.board_janitor import sync_reflection

    _tick_proposal(reflection, "no", "weekly-propose-2026-09-20")
    assert sync_reflection(reflection, vault) == 1
    assert not (vault / "OpenAugi" / "Tasks").exists()
    assert "- ✓ no" in reflection.read_text(encoding="utf-8")
