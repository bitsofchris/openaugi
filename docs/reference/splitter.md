...
6. **Empty / structural-only segments** (just `---`, empty checkboxes, URL-only lines, dataview queries) are dropped. A dangling anchor with no content above it is dropped too.
   - **Daily-journal template header.** The preamble before the first heading is dropped outright (not emitted as its own block) when it's *entirely* the daily note template's furniture: a dataview block plus the `#note-type/daily-journal` tag, a `[[My Taxonomy]]` link, and a `📥 Capture: [[...]]` link — no other content. This is scoped to notes carrying the `#note-type/daily-journal` tag; other note types with a leading dataview block are unaffected. A preamble with real content mixed in alongside the tag is kept as before.
