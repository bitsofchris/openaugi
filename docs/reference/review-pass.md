...
- **Weekly:** read `View - Dashboard.md`; answer the gravity nominations
  (inline note or zzz) — that's the entire review burden. Approving a
  nomination is the whole cost: the agent applies the drafted registration
  edit itself. Leaving one unanswered costs nothing (its age shows); a
  "not now" is just a slow no — there is no separate park state.
